      ...
      await db.Favorite.create({
        ...
        userId:
          req.session.userLogin
            .id /* si lo quiere como favorito("lo crea",corazón relleno) */,
      });
    }
...
const getAllProducts = async (req, res) => {
  try {
    ...
    const productsWithImage = products.map((product) => {
      product.image = `${req.protocol}://${req.get("host")}/img/products/${
        product.image
      }`;
      return product;
    });
...
const createProduct = async (req, res) => {
  try {
    const { name, price, discount, description, brandId, categoryId } =
      req.body;
...
const updateProduct = async (req, res) => {
  try {
    const { name, discount, price, description, brandId, categoryId } =
      req.body;
