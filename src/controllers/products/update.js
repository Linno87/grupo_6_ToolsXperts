...
module.exports = (req, res) => {
  ...
  const { name, category, brand, price, discount, description, rubroId } =
    req.body;
  ...
  if (errors.isEmpty()) {
    ...
    db.Product.findByPk(id, {
      ...
      .then((product) => {
        req.files
          .image /*si viene la imagen por req.files.image y es la misma que se encuentra en la carpeta públic, borrala*/ &&
        ...
        db.Product.update(
          {
            ...
            image: req.files.image
              ...
              : product.image /* actualiza la "imagen" principal */,
          },
          {
            where: {
          ...
          if (req.files.images) {
            ...
            db.Image.destroy({
              ...
              const images = req.files.images.map((file) => {
                /* crea una constante con el los nombres(array) que llegan del imput(por el name="images") y los mapea, retornando un nuevo array con los datos reemplazados */
                return {
                  file: file.filename,
                  main: false,
